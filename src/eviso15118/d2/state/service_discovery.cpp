...
message_2::ServiceDiscoveryResponse handle_request(const message_2::ServiceDiscoveryRequest& req,
    ...
    std::vector<message_2::ServiceDiscoveryResponse::Service> energy_services_list;
    std::vector<message_2::ServiceDiscoveryResponse::Service> vas_services_list;
...
FsmSimpleState::HandleEventReturnType ServiceDiscovery::handle_event(AllocatorType& sa, FsmEvent ev) {
    ...
    if (const auto req = variant->get_if<message_2::ServiceDiscoveryRequest>()) {
