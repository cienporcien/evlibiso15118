...
void MessageExchange::set_request(std::unique_ptr<message_2_sap::Variant> new_request) {
    if (request) {
...
std::unique_ptr<message_2_sap::Variant> MessageExchange::get_request() {
    if (not request) {
...
Context::Context(MessageExchange& message_exchange_, const std::optional<ControlEvent>& current_control_event_,
                 session_2_sap::feedback::Callbacks feedback_callbacks, bool& stopping_, session_2_sap::SessionLogger& logger,
                 const d2_sap::SessionConfig& config_) :
...
std::unique_ptr<message_2_sap::Variant> Context::get_request() {
