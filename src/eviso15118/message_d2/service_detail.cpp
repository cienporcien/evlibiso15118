...
ServiceDetailResponse::ParameterSet::ParameterSet(uint16_t _id, const DcParameterList& list) {
    ...
    if (list.control_mode == message_2::ControlMode::Scheduled) {
        mobility.value = static_cast<int32_t>(message_2::MobilityNeedsMode::ProvidedByEvcc);
    } else {
...
ServiceDetailResponse::ParameterSet::ParameterSet(uint16_t _id, const InternetParameterList& list) {
    ...
    protocol.value = message_2::from_Protocol(list.protocol);
...
struct ParamterValueVisitor {
    ...
    void operator()(const message_2::RationalNumber& in) {
