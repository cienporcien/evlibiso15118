...
template <> void convert(const struct iso2_ChargeParameterDiscoveryReqType& in, DC_ChargeParameterDiscoveryRequest& out) {
...
void convert(const DC_ChargeParameterDiscoveryResponse& in, struct iso2_ChargeParameterDiscoveryResType& out) {
    ...
    //iso15118_car    pybind11_init_everestpy(pybind11::module_&)::<lambda(const string&)> :: EXIDecodingError (Exception): Exception: javax.xml.bind.UnmarshalException    //RDB From Josev, we are getting a 
    ...
    out.SAScheduleList.SAScheduleTuple.array[0].PMaxSchedule.PMaxScheduleEntry.array[0].RelativeTimeInterval.duration=86400;
    out.SAScheduleList.SAScheduleTuple.array[0].PMaxSchedule.PMaxScheduleEntry.array[0].RelativeTimeInterval.duration_isUsed=true;
    out.SAScheduleList.SAScheduleTuple.array[0].PMaxSchedule.PMaxScheduleEntry.array[0].RelativeTimeInterval_isUsed=true;
    ...
    out.SAScheduleList.SAScheduleTuple.array[0].PMaxSchedule.PMaxScheduleEntry.array[0].PMax.Unit=iso2_unitSymbolType_W;
    ...
    //RDB For some reason Josev doesn't like the EXI produced. Match the values from the EVSEV2G version exactly, since it works.
    // we get this exception from Josev
    //iso15118_car    pybind11_init_everestpy(pybind11::module_&)::<lambda(const string&)> :: EXIDecodingError (Exception): Exception: javax.xml.bind.UnmarshalException
