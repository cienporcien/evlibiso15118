...
#include <eviso15118/detail/d20/state/acdp_disconnect.hpp>
...
message_20::ACDP_DisconnectRequest ACDP_Disconnect::setup_request(const d20::Session &session)
{
    message_20::ACDP_DisconnectRequest req;
...
void ACDP_Disconnect::enter() {
    ...
    auto req = ACDP_Disconnect::setup_request(ctx.session);
    ctx.request(req);
...
FsmSimpleState::HandleEventReturnType ACDP_Disconnect::handle_event(AllocatorType& sa, FsmEvent ev) {
    ...
    if (const auto res = variant->get_if<message_20::ACDP_DisconnectResponse>())
    {
        if (res->processing == message_20::Processing::Ongoing)
        {
            ...
            const auto req = ACDP_Disconnect::setup_request(ctx.session);
            ctx.request(req);
    ...
    {
        ctx.log("expected ACDP_DisconnectRes! But code type id: %d", variant->get_type());
