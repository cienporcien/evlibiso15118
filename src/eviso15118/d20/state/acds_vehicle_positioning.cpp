...
#include <eviso15118/d20/state/acds_vehicle_positioning.hpp>
#include <eviso15118/d20/state/authorization_setup.hpp>

#include <eviso15118/detail/d20/context_helper.hpp>
#include <eviso15118/detail/d20/state/acds_vehicle_positioning.hpp>
...
message_20::ACDS_VehiclePositioningRequest ACDS_VehiclePositioning::setup_request(const d20::Session &session, const bool ev_mobility_status)
{
    message_20::ACDS_VehiclePositioningRequest req;
...
void ACDS_VehiclePositioning::enter() {
    ...
    auto req = ACDS_VehiclePositioning::setup_request(ctx.session, false);
    ctx.request(req);
}
...
FsmSimpleState::HandleEventReturnType ACDS_VehiclePositioning::handle_event(AllocatorType& sa, FsmEvent ev) {
    ...
    if (const auto res = variant->get_if<message_20::ACDS_VehiclePositioningResponse>()) {
        ...
        if (res->processing == message_20::Processing::Ongoing)
        {
            ...
            const auto req = ACDS_VehiclePositioning::setup_request(ctx.session, vehicle_immobilized);
            ctx.request(req);
        ...
        ctx.log("expected ACDS_VehiclePositioningRes! But code type id: %d", variant->get_type());
