...
#include <eviso15118/detail/d20/state/acdp_connect.hpp>
...
message_20::ACDP_ConnectRequest ACDP_Connect::setup_request(const d20::Session &session)
{
    message_20::ACDP_ConnectRequest req;
...
void ACDP_Connect::enter() {
    ...
    auto req = ACDP_Connect::setup_request(ctx.session);
    ctx.request(req);
...
FsmSimpleState::HandleEventReturnType ACDP_Connect::handle_event(AllocatorType& sa, FsmEvent ev) {
    ...
    if (const auto res = variant->get_if<message_20::ACDP_ConnectResponse>()) {
        ...
        if (res->processing == message_20::Processing::Ongoing)
        {
            ...
            const auto req = ACDP_Connect::setup_request(ctx.session);
            ctx.request(req);
        ...
        ctx.log("expected ACDP_ConnectRes! But code type id: %d", variant->get_type());
